...
cmc = coinmarketcapapi.CoinMarketCapAPI('ded7779e-c28f-4742-9265-923df12734d8')
...
class ActionShowCurrency(Action):
    """Show the currency of the crypto API site"""
    ...
    async def run(
        ...
        try:
            ...
            data = str(tools.data[0]['amount']) + ' ' + first_currency_id + ' = ' + str(tools.data[0]['quote'][second_currency_id]['price']) + ' ' + second_currency_id
            dispatcher.utter_message(text=f'{data}')
            slots_to_reset = ["first_currency_id", "second_currency_id"]
            return [SlotSet(slot, None) for slot in slots_to_reset]
        except:
            dispatcher.utter_message(response="utter_conversion_request_error")
            slots_to_reset = ["first_currency_id", "second_currency_id"]
            return [SlotSet(slot, None) for slot in slots_to_reset]